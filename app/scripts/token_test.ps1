...
function Assert-TokenResponse {
  ...
  if ($RequireUser) {
    ...
    if ($ExpectedUsername -ne "" -and ($null -eq $Response.Body -or $null -eq $Response.Body.data -or $Response.Body.data.username -ne $ExpectedUsername)) {
      $errors += "username expected $ExpectedUsername, got $($Response.Body.data.username)"
    }
  }
...
$username = "token_ok_$runId"
$password = "123456"

Write-Host "Running TokenTest against $BaseUrl"

$registerResp = Invoke-JsonApi -Method POST -Path "/api/v1/user/register" -Body @{
  username = $username
  password = $password
}
if ($registerResp.StatusCode -ne 200 -or $null -eq $registerResp.Body -or [int]$registerResp.Body.code -ne 0) {
  throw "TokenTest setup failed: seed user could not be registered. Raw: $($registerResp.Raw)"
}
...
if ($loginResp.StatusCode -ne 200 -or $null -eq $loginResp.Body -or [int]$loginResp.Body.code -ne 0 -or [string]::IsNullOrWhiteSpace([string]$loginResp.Body.token)) {
  throw "TokenTest setup failed: seed user could not log in. Raw: $($loginResp.Raw)"
}
...
Assert-TokenResponse -Id "M1-UT-013" -Title "valid token" -Response $resp -ExpectedHttpStatus 200 -ExpectedCode 0 -RequireUser -ExpectedUsername $username

$resp = Invoke-JsonApi -Method GET -Path "/api/v1/user/me" -Headers @{
  Authorization = "Bearer   $token   "
}
Assert-TokenResponse -Id "M1-UT-014" -Title "token with leading and trailing spaces" -Response $resp -ExpectedHttpStatus 200 -ExpectedCode 0 -RequireUser -ExpectedUsername $username

$resp = Invoke-JsonApi -Method GET -Path "/api/v1/user/me" -Headers @{
  Authorization = "Bearer "
}
Assert-TokenResponse -Id "M1-UT-015" -Title "empty token" -Response $resp -ExpectedHttpStatus 401 -ExpectedCode 401

$resp = Invoke-JsonApi -Method GET -Path "/api/v1/user/me" -Headers @{
  Authorization = "Bearer unknown"
}
Assert-TokenResponse -Id "M1-UT-016" -Title "unknown token" -Response $resp -ExpectedHttpStatus 401 -ExpectedCode 401

Write-Host ""
Write-Host "TokenTest summary: Total=$script:Total Passed=$script:Passed Failed=$script:Failed"

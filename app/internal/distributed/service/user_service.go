...
func (s *UserService) Register(_ context.Context, req *contracts.RegisterRequest) (*contracts.BaseResponse, error) {
	...
	if !validUsername(username) || len(req.Password) < 6 {
	...
	if s.db != nil {
		var exists userRow
		if err := s.db.Where("BINARY username = ?", username).First(&exists).Error; err == nil {
			return &contracts.BaseResponse{Code: 409, Message: "username already exists"}, nil
		} else if !errors.Is(err, gorm.ErrRecordNotFound) {
			log.Printf("user-service register query failed: %v", err)
			return &contracts.BaseResponse{Code: 500, Message: "register query failed"}, nil
		}
	}
	...
	if s.db != nil {
		...
		if err := s.db.Where("BINARY username = ?", username).First(&persisted).Error; err == nil && persisted.ID > 0 {
			row.ID = persisted.ID
		}
	} else {
		s.nextID++
...
func (s *UserService) Login(_ context.Context, req *contracts.LoginRequest) (*contracts.LoginResponse, error) {
	username := strings.TrimSpace(req.Username)
	if !validUsername(username) || req.Password == "" {
	...
	if !ok && s.db != nil {
		var dbRow userRow
		if err := s.db.Where("BINARY username = ?", username).First(&dbRow).Error; err == nil {
			row = dbRow
			ok = true
			s.mu.Lock()
